...
export default function InvRequests() {
  ...
  function updateFormItemQty(idx, val) {
    setFormItems(prev => prev.map((item, i) => i === idx ? { ...item, requestedQty: val } : item))
  }
  ...
  async function handleIssue() {
    ...
    try {
      ...
      const reqRef = doc(db, 'invRequests', issueRequest.id)
      batch.update(reqRef, { items: updatedItems, status: newStatus })
      ...
      for (let idx = 0; idx < issueRequest.items.length; idx++) {
        ...
        await addDoc(collection(db, 'invLedger'), {
  ...
  const filteredHistory = requests.filter(r => {
    ...
    if (filterFrom) {
      ...
      if (r.date.toDate() < from) return false
    }
    if (filterTo) {
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4">
      ...
      <div className="flex gap-2 border-b border-gray-200">
        {['new', canIssue && 'issue', 'history'].filter(Boolean).map(tab => (
          ...
          >
            {tab === 'new' ? 'New Request' : tab === 'issue' ? `Issue Items${pendingRequests.length ? ` (${pendingRequests.length})` : ''}` : 'History'}
          </button>
        ))}
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
          ...
          <div ref={searchRef} className="relative">
            ...
            {showSearchDropdown && searchResults.length > 0 && (
              <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-56 overflow-y-auto">
                {searchResults.map(mat => (
                  <button
                    key={mat.id}
                    onMouseDown={() => addMaterialToForm(mat)}
            ...
            {showSearchDropdown && searchResults.length === 0 && searchTerm && (
              <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 px-4 py-3 text-sm text-gray-500">
                No materials found
              </div>
            )}
          </div>
                  ...
                  {formItems.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="py-2 pr-4">
                        <span className={`font-medium ${item.currentStock <= 0 ? 'text-red-600' : 'text-gray-700'}`}>
                          {item.currentStock}
                        </span>
                      </td>
                      <td className="py-2 pr-4">
                        <input
                          ...
                          onChange={e => updateFormItemQty(idx, e.target.value)}
                  ...
                  {filteredHistory.map(req => (
                    ...
                    >
                      <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                        {format(req.date.toDate(), 'dd MMM yyyy')}
                      </td>
      ...
      {issueRequest && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 space-y-4">
            ...
            {issueRequest.notes && (
              <p className="text-sm text-gray-500 border-t pt-2">Notes: {issueRequest.notes}</p>
            )}

            <div className="flex gap-3 justify-end">
              <button
                onClick={() => setIssueRequest(null)}
                className="border border-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-50"
              >
                Cancel
              </button>
