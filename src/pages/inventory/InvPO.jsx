...
export default function InvPO() {
  ...
  useEffect(() => {
    getDoc(doc(db, 'restaurantSettings', 'main'))
      .then(snap => { if (snap.exists()) setRestaurantName(snap.data().name || '') })
      .catch(() => {})
  }, [])

  function handlePrint(po) {
    const dateStr = po.date instanceof Timestamp ? format(po.date.toDate(), 'dd MMM yyyy') : '—'
    const poNum = po.id.slice(-6).toUpperCase()
    const rows = (po.items || []).map((it, i) =>
      `<tr><td>${i + 1}</td><td>${it.materialName}</td><td style="text-align:center">${it.uom}</td><td style="text-align:center">${it.orderedQty}</td></tr>`
    ).join('')
    const html = `<!DOCTYPE html><html><head><title>PO #${poNum}</title>
    <style>
      body { font-family: Arial, sans-serif; padding: 32px; color: #111; }
      h2 { margin: 0 0 4px; } .sub { color: #666; font-size: 13px; margin-bottom: 24px; }
      .meta { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 24px; font-size: 13px; }
      .meta span { color: #888; } .meta strong { display: block; }
      table { width: 100%; border-collapse: collapse; font-size: 13px; }
      th { background: #f5f5f5; text-align: left; padding: 8px 12px; border-bottom: 2px solid #ddd; }
      td { padding: 8px 12px; border-bottom: 1px solid #eee; }
      .notes { margin-top: 16px; font-size: 13px; color: #555; }
      .footer { margin-top: 32px; font-size: 12px; color: #aaa; text-align: center; }
      @media print { body { padding: 16px; } }
    </style></head><body>
    <h2>${restaurantName || 'Purchase Order'}</h2>
    <div class="sub">Purchase Order</div>
    <div class="meta">
      <div><span>PO Number</span><strong>#${poNum}</strong></div>
      <div><span>Date</span><strong>${dateStr}</strong></div>
      <div><span>Vendor</span><strong>${po.vendorName}</strong></div>
      ${po.vendorPhone ? `<div><span>Phone</span><strong>${po.vendorPhone}</strong></div>` : ''}
      <div><span>Status</span><strong style="text-transform:capitalize">${po.status}</strong></div>
    </div>
    <table>
      <thead><tr><th>#</th><th>Material</th><th style="text-align:center">UOM</th><th style="text-align:center">Qty</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    ${po.notes ? `<div class="notes"><strong>Notes:</strong> ${po.notes}</div>` : ''}
    <div class="footer">Generated by ${restaurantName || 'Restaurant Manager'}</div>
  ...
  function handleWhatsApp(po) {
    const dateStr = po.date instanceof Timestamp ? format(po.date.toDate(), 'dd MMM yyyy') : '—'
    const poNum = po.id.slice(-6).toUpperCase()
    const itemLines = (po.items || []).map((it, i) => `${i + 1}. ${it.materialName} — ${it.orderedQty} ${it.uom}`).join('\n')
    const msg = [
    ...
    const phone = (po.vendorWhatsapp || po.vendorPhone || '').replace(/\D/g, '')
    const url = phone
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto space-y-4">
            ...
            <div className="flex flex-wrap gap-2 justify-between pt-2">
              <div className="flex gap-2">
                <button onClick={() => handlePrint(viewPO)}
                  className="border border-gray-300 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center gap-1">
                  🖨️ Print
                </button>
                <button onClick={() => handlePrint(viewPO)}
                  className="border border-gray-300 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center gap-1">
                ...
                <button onClick={() => handleWhatsApp(viewPO)}
                  className="border border-green-400 text-green-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-green-50 flex items-center gap-1">
                  💬 WhatsApp
                </button>
              </div>
              <div className="flex gap-2">
